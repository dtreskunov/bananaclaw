...
/**
 * Sync container-written `processing_ack` rows into `messages_in.status`.
 * Returns the ids that *actually* transitioned to `completed` on this call
 * (i.e. the UPDATE changed a row) so callers can react exactly once per
 * completion even though completed `processing_ack` rows persist across ticks.
 */
...
/**
 * Read the container's current tool-in-flight state, if any. Host sweep reads
 * this to widen stuck-detection tolerance while Bash is running with a long
 * declared timeout.
 */
