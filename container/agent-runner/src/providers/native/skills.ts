...
export class NativeSkillRegistry {
  ...
  instructions(): string | null {
    ...
    return [
      ...
      'a skill below, call `skill` before acting and follow the returned instructions.',
      'Use the optional `path` argument to read files referenced by that skill.',
      'Skill `allowed-tools` metadata may use Claude names: Bash/Read/Write/Edit map',
      'to `bash`/`read`/`write`/`edit`, and Server(tool) maps to',
