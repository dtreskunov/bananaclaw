...
WebAssembly runs from workspace-hosted `.wasm` files, but `eval()`, `new Function()`, and web workers are blocked; prefer single-threaded builds that avoid dynamic code generation.
